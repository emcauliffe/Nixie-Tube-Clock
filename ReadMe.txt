ATmega238p-Powered Nixie Tube Clock

This clock uses the ATmega238p to show the current time on 6 nixie tubes. The Amega238p pulls time data from the RTC and pushes it to the shift registers. The shift registers then transmit this data to the nixie tube drivers. The ATmega238p also controls a boost converter that increases the 5V power source to 170-200V. The duty cycle of the PWM pin controlling the boost converter can be adjusted by a potentiometer. There is also an option to turn the tubes completely off. A switch on the back adds or subtracts one hour from the current time for easy DST adjustment.
